...
def viewbookingagents(request):
    query = "SELECT b.email, b.booking_agent_ID, COUNT(bk.ticket_id) FROM booking_agent b, bookings bk WHERE bk.booking_date >= ADDDATE(DATE(NOW()), INTERVAL -48 MONTH) AND b.booking_agent_ID = bk.booking_agent_id GROUP BY b.email, b.booking_agent_ID ORDER BY COUNT(bk.ticket_id) DESC "
    cursor = conn.cursor()
    cursor.execute(query)
    top5_agent_onem = []
    ...
    query = "SELECT b.email, b.booking_agent_ID, COUNT(bk.ticket_id) FROM booking_agent b, bookings bk WHERE bk.booking_date >= ADDDATE(DATE(NOW()), INTERVAL -36 MONTH) AND b.booking_agent_ID = bk.booking_agent_id GROUP BY b.email, b.booking_agent_ID ORDER BY COUNT(bk.ticket_id) DESC "
    cursor = conn.cursor()
    cursor.execute(query)
    top5_agent_oney = []
    temp = cursor.fetchone()
